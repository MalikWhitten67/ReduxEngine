...
class ReduxEngine {
    constructor(containerId, width, height, frameRate = 60) {
        ...
        this.keyStates = {}; // Track key states (pressed or released)
     ...
     * });
     */
    handleInput(key, startCallback, stopCallback) {
        // StartCallback is called when the key is pressed
        // StopCallback is called when the key is released
        this.keyStates[key] = {
            isPressed: false,
            startCallback,
            stopCallback,
    ...
    removeInputListeners(key) {
        if (key) {
            window.removeEventListener('keydown', this.keyStates[key].startCallback);
            window.removeEventListener('keyup', this.keyStates[key].stopCallback);
        } else {
            for (const k in this.keyStates) {
                if (this.keyStates.hasOwnProperty(k)) {
                    window.removeEventListener('keydown', this.keyStates[k].startCallback);
                    window.removeEventListener('keyup', this.keyStates[k].stopCallback);
                }
            }
        }
    }
    ...
    start() {
        if (!this.isRunning) {
            ...
            throw new Error('Too many instances of ReduxEngine start should only be called once')
        }
    }
    /**
...
class Camera {
    constructor(container, targetEntity) {
        this.container = container;
        this.targetEntity = targetEntity;
        this.voidColor = 'black';
        this.container.style.overflow = 'hidden';
        this.offsetX = 0;
        this.offsetY = 0;
    }
...
const isColliding = (elementA, elementB, sensitivity) => {
   ...
   console.log(rectA, rectB)
    ...
    if (
        ...
        const overlapY = (heightA + heightB) / 2 + adjustedMinDistance - Math.abs(dy);
...
function parseTextureScript(textureScript) {
    ...
    const functions = {
        ...
        gradient: (direction, color1, color2) => `linear-gradient(${direction}, #${color1}, #${color2})`,
        ...
        bgGradient: (direction, color1, color2) => `background-image: linear-gradient(${direction}, #${color1}, #${color2});`,
        bgGradientRadial: (color1, color2) => `background-image: radial-gradient(#${color1}, #${color2});`,
        bgGradientRadialCircle: (color1, color2) => `background-image: radial-gradient(circle, #${color1}, #${color2});`,
        bgGradientRadialEllipse: (color1, color2) => `background-image: radial-gradient(ellipse, #${color1}, #${color2});`,
        bgGradientRadialClosestSide: (color1, color2) => `background-image: radial-gradient(closest-side, #${color1}, #${color2});`,
    };
...
class Scene {
    ...
    removeUIElement(uiElement) {
        this.uiElements = this.uiElements.filter((element) => element !== uiElement);
        uiElement.element.remove();
    }
    ...
    addCustomInput(key, startCallback, stopCallback) {
        this.customInputs[key] = {
            startCallback,
            stopCallback,
        };

        this.engine.handleInput(key, startCallback, stopCallback);
    }
...
class StartMenuScene extends Scene {
    constructor(engine) {
        ...
        `
        this.startui = new UI(100, 100, 200, 50, div, 'startui');
...
class GameScene extends Scene {
    constructor(engine) {
        ...
        const camera = new Camera(this.engine.container, this.player, 'white');
